                ...
                with open("people_data.csv",newline="",encoding='utf-8') as csvfile:
                    ...
                    if row_id_no_extract is not None:
                        ...
                        with open('people_data.csv', mode='w', newline='',encoding="utf-8") as csvfile:
                            ...
                            writer.writerows(rows)
                        # Create Dictionary
                        with open('people_data.csv',  'r', encoding='utf-8') as f:
...
def save_new_people():
    ...
    if not all([number_id_save, name_save, birthday_save, phone_save, major_save]):
        ...
        if not os.path.exists("people_checked"):
            os.makedirs("people_checked")
        
        # Copy image
        file_path = "check_image.jpg"
        file_path_dst = "people_checked/check_image_copy.jpg"
        shutil.copy(file_path, file_path_dst)
        
        #handle name image -> save
        name_handle = unidecode(name_save).replace(" ", "").lower()
        current_name = "people_checked/check_image_copy.jpg"
        new_name = "people_checked/" + str(number_id_save) + "_" + name_handle + ".jpg"
...
def signup_advise():
    ...
    school = data['school']
    major = data['major'] 
    with open('signup_advise.csv',  'r', encoding='utf-8') as f:
        id = len(f.readlines())
    with open('signup_advise.csv', mode='a', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
